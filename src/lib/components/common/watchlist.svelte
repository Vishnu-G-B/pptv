<script>
    ...
    import { openLightbox } from '$lib/stores/modalStore.js';
    ...
    function handleMovieCardTrailer() {
        if (trailerPlaying) { revertMovieCardTrailerPlaying(); } else { playMovieCardsTrailer(); }
    }
    ...
    function playMovieCardsTrailer() {
        ...
        Flip.from(trailerMovieState, {
            ...
            onComplete: () => {
                ...
                movieTrailerTimeline.to('#movie-trailer-playing', { duration: 0.75, opacity: 1, ease: 'sine' }, '<');
            }
        });
    }
    ...
    function revertMovieCardTrailerPlaying() {
        ...
        gsap.to('#movie-trailer-playing', {
            duration: 0.75, opacity: 0, scale: 0.5, ease: 'sine',
            onComplete: () => { document.getElementById('movie-trailer-playing').remove(); }
        });
        Flip.from(trailerMovieState, {
            duration: 0.75, ease: 'sine',
            onComplete: () => { trailerPlaying = false; }
        })
    }
    ...
    function getTranslatePercentage(element) {
        ...
        return { x: matrix.m41 / parseInt(w) * 100, y: matrix.m42 / parseInt(h) * 100, rotation: angle };
    }
...
<div class="min-h-screen h-fit w-full flex flex-col relative bg-background wonderfull-trigger items-start justify-start pt-5">
    ...
    <div class="h-screen w-full flex-col items-center justify-center hidden lg:flex gap-2 sticky top-0">
        ...
        <div class="text-7xl xl:text-8xl 2xl:text-9xl brand-font font-bold text-brand-orange relative italic overflow-hidden flex flex-row">
            <div class="relative overflow-visible"><p class="opacity-0 ">WONDERFULL</p></div>
        ...
        <div class="flex flex-row w-full gap-8 px-10 mt-10 max-w-[1100px]">
            <div class="w-1/2 flex flex-col items-start justify-start p-1">
                <p class="text-lg xl:text-xl text-primary font-bold italic text-left w-full mb-2">
                    Watch Impactful Films Starting @ONE Rupee
                </p>
                <p class="text-base font-light text-surface w-full">
            ...
            <div class="w-1/2 flex flex-col items-start justify-start p-1">
                <p class="text-lg xl:text-xl text-brand-orange font-bold italic text-left w-full mb-2">
                    Since 2012
                </p>
                <p class="text-base font-light text-surface w-full">
                    Since 2012, our Film Pedagogy programs at Purple People Labs have mentored children and teachers
                    to create purpose-driven cinema as young content creators. Our films have featured on PM Vidya
                    Channels of Ladakh and won accolades at International Film Festivals including Seattle,
                    I&amp;B Ministry's Golden Elephant, MICE Film Festival Venice, Miami International Film Festival
                    &amp; the prestigious Cannes 2025. Soon available on Swadeshplex at just ₹1.
                </p>
            </div>
        </div>
    </div>

    <!-- Mobile carousel (original — unchanged) -->
    <div class="min-h-screen h-fit lg:hidden w-full bg-background flex flex-col items-center pt-5 relative mb-[150px] ">
                ...
                <div class="flex flex-row h-[300px] items-end justify-center absolute">
                    <div class="w-[200px] flex-shrink-0 movie-cards movie-card-{index+1} flex flex-row-reverse"
                         style="translate: translate3d({110 * index}%, 0, 0); opacity: {index === 0 ? 1 : 0.7};
                                        scale: {index === 0 ? 1 : 0.9}">
                        <button class="h-full w-full text-left" on:click={() => openLightbox(VIMEO_PLACEHOLDER)}>
                            <div class="h-full w-full relative">
                                ...
                                <div class="absolute inset-0 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity">
                                    <div class="bg-black/60 rounded-full p-3">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-7 w-7 text-white" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>
                                    </div>
                                </div>
                            </div>
                        </button>
        ...
        <div class="bg-surface h-fit w-full mt-[150px] relative ">
            <div class="relative h-[150px]">
                <svg class="absolute -top-[90%] left-0 w-full h-[150px]" preserveAspectRatio="none" viewBox="0 0 1440 320" xmlns="http://www.w3.org/2000/svg">
                    <path d="M0,0L34.3,32C68.6,64,137,128,206,154.7C274.3,181,343,171,411,176C480,181,549,203,617,186.7C685.7,171,754,117,823,112C891.4,107,960,149,1029,186.7C1097.1,224,1166,256,1234,256C1302.9,256,1371,224,1406,208L1440,192L1440,320L1405.7,320C1371.4,320,1303,320,1234,320C1165.7,320,1097,320,1029,320C960,320,891,320,823,320C754.3,320,686,320,617,320C548.6,320,480,320,411,320C342.9,320,274,320,206,320C137.1,320,69,320,34,320L0,320Z" fill="#FF9900FF" fill-opacity="1"></path>
                </svg>
                <svg class="absolute -top-[80%] left-0 w-full h-[150px]" preserveAspectRatio="none" viewBox="0 0 1440 320" xmlns="http://www.w3.org/2000/svg">
                    <path d="M0,0L34.3,32C68.6,64,137,128,206,154.7C274.3,181,343,171,411,176C480,181,549,203,617,186.7C685.7,171,754,117,823,112C891.4,107,960,149,1029,186.7C1097.1,224,1166,256,1234,256C1302.9,256,1371,224,1406,208L1440,192L1440,320L1405.7,320C1371.4,320,1303,320,1234,320C1165.7,320,1097,320,1029,320C960,320,891,320,823,320C754.3,320,686,320,617,320C548.6,320,480,320,411,320C342.9,320,274,320,206,320C137.1,320,69,320,34,320L0,320Z" fill="#250025" fill-opacity="1"></path>
                </svg>
            </div>
            <div class="h-fit bg-surface pb-7 w-full px-5 md:px-28 flex flex-col gap-5 items-center ">
                <div class="flex flex-col h-fit w-fit items-center justify-center gap-2">
                    <div class="flex flex-row gap-5 z-[2] mt-5">
                        <button class="h-8 w-8 rounded-full bg-on-surface/50 backdrop-blur-xl wow-controller-left flex items-center justify-center" on:click={movieCarouselLeft}>
                            <svg height="24" viewBox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><g><path d="M12,2A10,10,0,1,0,22,12,10.011,10.011,0,0,0,12,2Zm0,18a8,8,0,1,1,8-8A8.009,8.009,0,0,1,12,20Z"/><polygon points="12.707 8.707 11.293 7.293 6.586 12 11.293 16.707 12.707 15.293 10.414 13 16 13 16 11 10.414 11 12.707 8.707"/></g></svg>
                            <p class="hidden">Go left</p>
                        </button>
                        <button class="h-8 w-8 rounded-full bg-on-surface/50 backdrop-blur-xl woww-controller-right flex items-center justify-center" on:click={() => {movieCarouselRight()}}>
                            <svg height="24" viewBox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><g><path d="M12,2A10,10,0,1,0,22,12,10.011,10.011,0,0,0,12,2Zm0,18a8,8,0,1,1,8-8A8.009,8.009,0,0,1,12,20Z"/><polygon points="11.293 8.707 13.586 11 8 11 8 13 13.586 13 11.293 15.293 12.707 16.707 17.414 12 12.707 7.293 11.293 8.707"/></g></svg>
                ...
                <div class="h-fit w-full">
                    <p class="brand-font font-thin text-background text-xl">
                        Our films are not just stories; they are windows into the world from a fresh and unique
                        perspective—<span class="text-brand-orange font-bold italic">The Children's Perspective.</span> Each
                        film reflects the hopes, challenges and life-experiences of young minds, bringing their untold stories to life.
                    </p>
                </div>
            ...
            <div class="relative h-[50px] ">
                <svg class="absolute -bottom-[280%] -rotate-180 left-0 w-full h-[150px] " preserveAspectRatio="none" viewBox="0 0 1440 320" xmlns="http://www.w3.org/2000/svg">
                    <path d="M0,0L34.3,32C68.6,64,137,128,206,154.7C274.3,181,343,171,411,176C480,181,549,203,617,186.7C685.7,171,754,117,823,112C891.4,107,960,149,1029,186.7C1097.1,224,1166,256,1234,256C1302.9,256,1371,224,1406,208L1440,192L1440,320L1405.7,320C1371.4,320,1303,320,1234,320C1165.7,320,1097,320,1029,320C960,320,891,320,823,320C754.3,320,686,320,617,320C548.6,320,480,320,411,320C342.9,320,274,320,206,320C137.1,320,69,320,34,320L0,320Z" fill="#FF9900FF" fill-opacity="1"></path>
                </svg>
                <svg class="absolute -bottom-[250%] -rotate-180 left-0 w-full h-[150px] " preserveAspectRatio="none" viewBox="0 0 1440 320" xmlns="http://www.w3.org/2000/svg">
                    <path d="M0,0L34.3,32C68.6,64,137,128,206,154.7C274.3,181,343,171,411,176C480,181,549,203,617,186.7C685.7,171,754,117,823,112C891.4,107,960,149,1029,186.7C1097.1,224,1166,256,1234,256C1302.9,256,1371,224,1406,208L1440,192L1440,320L1405.7,320C1371.4,320,1303,320,1234,320C1165.7,320,1097,320,1029,320C960,320,891,320,823,320C754.3,320,686,320,617,320C548.6,320,480,320,411,320C342.9,320,274,320,206,320C137.1,320,69,320,34,320L0,320Z" fill="#250025" fill-opacity="1"></path>
                </svg>
            </div>
        </div>
    ...
    <!--  Desktop GRID — original Card3D layout, clicking opens vimeo lightbox via store -->
        ...
        <div class="grid-cols-[repeat(3,minmax(0,1fr))] hidden lg:grid lg:gap-[40px] xl:grid-cols-[repeat(4,minmax(0,1fr))] xl:gap-[30px] px-10">
            {#each {length: totalMovieCards} as _, index}
                {#if index === 4 && innerWidth >= 1280}
                    <div class="opacity-0 rounded-xl lg:p-3 xl:p-4 h-full transform-style-preserve-3d shadow-xl"></div>
                {/if}
                <button
                    class="text-left focus:outline-none group/poster"
                    on:click={() => openLightbox(VIMEO_PLACEHOLDER)}
                    aria-label="Play film {movieCardsHeading[index]}"
                >
                    <div class="relative">
                        ...
                        <!-- Play icon on hover -->
                        <div class="absolute inset-0 flex items-center justify-center opacity-0 group-hover/poster:opacity-100 transition-opacity duration-300 pointer-events-none">
                            <div class="bg-black/60 rounded-full p-4 mt-[-40px]">
